...
export const CartProvider:FC<PropsWithChildren> = ({ children }) => {
   ...
   const loadCookies = async() => {
      try {
         ...
         await dispatch({ type: '[Cart] - LoadCart from cookies | storage', payload: cookieCart }) ;
      } catch (error) {
   ...
   //Efecto leer de cookie y cargar
   useEffect(() => {
      loadCookies();
   }, []);
   ...
   const addProductToCart = (product: ICartProduct) => {
      ...
      //Acumukar
      const uppdatedProducts = state.cart.map( p => {
      ...
      dispatch({ type: '[Cart] - Update products in cart', payload: uppdatedProducts });
